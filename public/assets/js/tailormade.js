...
/* ================================
   🌍 TRANSFER BOOKING FORM SCRIPT
   By HaGer Hamed ❤️
================================ */

// المتغير الأساسي لتتبع الخطوة الحالية
let currentStep = 1;

/* ================================
   🔹 دالة عرض الخطوة الحالية
================================ */
function showStep(step) {
  // إخفاء كل الخطوات
  ...
  // تحديث شريط التنقل بالأعلى
  document.querySelectorAll('.step-nav div').forEach((nav, index) => {
    nav.classList.remove('active');
    if (index + 1 === step) nav.classList.add('active');
  });

  // إظهار أو إخفاء ملخص الرحلة في الـ Sidebar
  if (step > 1) {
    document.getElementById('trip-initial').style.display = 'none';
    document.getElementById('trip-summary').style.display = 'block';
    updateTripSummary();
...
/* ================================
   🔹 تفعيل زر "Next" في الخطوة 1
================================ */
function setupCitySelection() {
  const cityChecks = document.querySelectorAll('.city-check');
  const nextBtnStep1 = document.querySelector('.step-1-content .custom-btn');

  cityChecks.forEach(check => {
    check.addEventListener('change', () => {
      const anyChecked = Array.from(cityChecks).some(c => c.checked);
      nextBtnStep1.disabled = !anyChecked;
      updateTripSummary();
    });
  });
}
...
/* ================================
   🔹 خيارات الوقت (Exact / Approx / Not Sure)
================================ */
function setupTimeOptions() {
  ...
  optionRadios.forEach(radio => {
    radio.addEventListener('change', function () {
      ...
      // إظهار الصندوق الخاص بالخيار المختار
...
/* ================================
   🔹 عدادات الضيوف (Adults / Children / Infants)
================================ */
...
/* ================================
   🔹 تحديث ملخص الرحلة (Trip Summary)
================================ */
function updateTripSummary() {
  // المدن المختارة
  const selectedCities = Array.from(document.querySelectorAll('.city-check:checked'))
    .map(c => c.value)
    .join(', ');
  document.getElementById('selectCity').textContent = selectedCities || 'None';

  // الاسم والإيميل
  document.getElementById('travelerFullName').textContent =
    document.getElementById('full_name')?.value || 'None';
  document.getElementById('travelerEmail').textContent =
    document.getElementById('input_email')?.value || 'None';

  // رقم الهاتف
  const phoneCode = document.getElementById('phone_code')?.value || '';
  const phoneNum = document.getElementById('phone_number')?.value || '';
  document.getElementById('travelerPhoneNumber').textContent =
    phoneCode && phoneNum ? `+${phoneCode} ${phoneNum}` : 'Not selected';

  // الفندق
  document.getElementById('hotels').textContent =
    document.getElementById('hotel-input')?.value || 'Not selected';

  // عدد الضيوف
  const adults = document.getElementById('adult_count').value;
  const children = document.getElementById('children_count').value;
  const infants = document.getElementById('infant_count').value;
  document.getElementById('travelerNumbers').textContent =
    `${adults} Adults, ${children} Children, ${infants} Infants`;

  // الأسعار
  const inputMin = document.querySelector('.input-min');
  const inputMax = document.querySelector('.input-max');
  document.getElementById('travelerPrices').textContent =
    `${inputMin.value || 0} - ${inputMax.value || 0}`;

  // عدد الأيام أو التواريخ
  ...
  if (checkIn && checkOut) {
    document.getElementById('selectedDates').textContent = `${checkIn} to ${checkOut}`;
  } else if (month) {
    document.getElementById('selectedDates').textContent = month;
  } else {
    document.getElementById('selectedDates').textContent = 'Not selected';
  }

  document.getElementById('selectedDays').textContent = days || 'Not selected';
}

/* ================================
   🔹 تحديث تلقائي عند إدخال بيانات
================================ */
...
function setupCitySelection() {
  const cityChecks = document.querySelectorAll('.city-check'); // input[type="checkbox"]
  const nextBtnStep1 = document.querySelector('.step-1-content .custom-btn');

  cityChecks.forEach(check => {
    check.addEventListener('change', () => {
      const cityBox = check.closest('.city-box');
      ...
      // تفعيل زر Next إذا تم اختيار أي مدينة
      const anyChecked = Array.from(cityChecks).some(c => c.checked);
      nextBtnStep1.disabled = !anyChecked;
  ...
  // دعم النقر على label لتفعيل نفس التأثير
  document.querySelectorAll('.city-box label').forEach(label => {
    label.addEventListener('click', () => {
      const checkbox = label.previousElementSibling || label.querySelector('input[type="checkbox"]');
      setTimeout(() => {
        const cityBox = label.closest('.city-box');
        if (checkbox.checked) {
          cityBox.classList.add('active');
        } else {
          cityBox.classList.remove('active');
        ...
        updateTripSummary();
      }, 10); // تأخير بسيط للتأكد من تحديث حالة checkbox
    });
  });
}
